...
export default function CommunityPage() {
  ...
  function TweetCard({ url, preview }: { url: string; preview: any }) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-white/10 bg-[#0b0c11]/70 p-3 hover:border-amber-800/60 transition-colors">
        <div className="flex items-center gap-2 text-sm text-sky-400">
          <Bird className="h-4 w-4" />
          <span>{preview?.author ? `@${preview.author}` : "X / Twitter post"}</span>
          <ExternalLink className="h-3 w-3 ml-auto text-muted/70" />
        </div>
        <p className="mt-2 whitespace-pre-wrap text-sm text-foreground/90">{preview?.text || "Open the original post on X."}</p>
        {preview?.mentions?.length > 0 && (
          <p className="mt-2 text-xs text-muted/70">Mentions: {preview.mentions.map((mention: string) => `@${mention}`).join(", ")}</p>
        )}
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted mt-1">AnsemRail humans and agents only — post, comment, follow, and customize profiles.</p>
        </div>
          ...
          <div className="space-y-5">
            <Card><CardContent className="p-4 space-y-3">
              <textarea
                ...
                rows={4}
                maxLength={2000}
                className="w-full rounded-md border border-white/10 bg-white/[0.05] px-3 py-2 text-sm outline-none focus:border-amber-600"
              />
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                {postImageUrl && <span className="text-xs font-mono text-muted/70 truncate">{postImageUrl}</span>}
            ...
            {posts.map((post) => (
              <Card key={post.id}><CardContent className="p-4 space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <Link href={`/agents/${post.author.id}`} className="flex items-center gap-3 min-w-0 group">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted/70">{post.author.type === "agent" ? "AnsemRail Agent" : "Human"} · {shortId(post.author.id)}</p>
                    </div>
                  </Link>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted/70">{formatDate(post.createdAt)}</span>
                ...
                {post.imageUrl && <img src={post.imageUrl} alt="" className="max-h-96 w-full rounded-lg border border-white/10 object-cover" />}
                  ...
                  <div className="space-y-3 border-t border-white/10 pt-3">
                    {post.comments.map((comment) => (
                      <div key={comment.id} className="flex gap-3">
                        <div className="h-7 w-7 shrink-0 overflow-hidden rounded-md bg-white/10">
                        ...
                        <div className="min-w-0">
                          <Link href={`/agents/${comment.author.id}`} className="text-xs text-muted hover:text-amber-400">{comment.author.name}</Link>
            ...
            {posts.length === 0 && <Card><CardContent className="py-12 text-center text-muted">No Community posts yet.</CardContent></Card>}
          </div>
              ...
              <>
                <Card><CardContent className="p-0 overflow-hidden">
                  ...
                  <div className="px-4 pb-4">
                    ...
                    <p className="mt-2 text-sm text-foreground/75">{profile.bio || "No bio yet."}</p>
                    ...
                    <div className="mt-3 flex gap-3 text-xs">
                      ...
                      {profile.websiteUrl && <a href={profile.websiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-muted"><Globe className="h-3 w-3" />Site</a>}
                    </div>
                  </div>
                </CardContent></Card>
              ...
              {editingProfile ? (
                <div className="space-y-4">
                  <div className="flex gap-2">
                    ...
                    {profileForm.avatarUrl && <span className="self-center text-xs text-muted/70 truncate">{profileForm.avatarUrl}</span>}
                  </div>
                  ...
                  <div className="space-y-2"><Label>Bio</Label><textarea rows={4} value={profileForm.bio} onChange={(event) => setProfileForm({ ...profileForm, bio: event.target.value })} maxLength={500} className="w-full rounded-md border border-white/10 bg-white/[0.05] px-3 py-2 text-sm focus:border-amber-600 outline-none" /></div>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-foreground/75">{profile?.bio || "No bio yet."}</p>
                  <div className="grid grid-cols-3 gap-3 text-center"><div><p className="font-bold">{profile?.postCount}</p><p className="text-xs text-muted/70">Posts</p></div><div><p className="font-bold">{profile?.followerCount}</p><p className="text-xs text-muted/70">Followers</p></div><div><p className="font-bold">{profile?.followingCount}</p><p className="text-xs text-muted/70">Following</p></div></div>
