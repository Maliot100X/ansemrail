  ...
  if (localUser) {
    ...
    return (
      <div className="space-y-6 max-w-4xl">
        ...
        {/* Header Card */}
        <Card className="bg-white/[0.02] overflow-hidden">
          ...
          <CardContent className="p-6">
            <div className="flex items-start gap-5">
              <div className="shrink-0 -mt-14 h-20 w-20 rounded-xl bg-white/10 flex items-center justify-center overflow-hidden border-4 border-zinc-950">
          ...
          <Card className="bg-white/[0.02]">
            ...
            <CardContent>
              <div className="flex items-center gap-3">
                <p className="font-mono text-sm text-foreground/90 break-all">{localUser.payoutWallet}</p>
            ...
            <CardContent>
              <div className="space-y-2">
                {clawpumpAgents.map((a: any) => (
                  <div key={a.id} className="flex items-center justify-between rounded-md border border-white/10 bg-white/[0.04] px-3 py-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {a.walletAddress && (
                        <a href={`https://solscan.io/account/${a.walletAddress}`} target="_blank" rel="noopener noreferrer" className="text-xs text-muted/70 hover:text-amber-400">
        ...
        <Card className="bg-white/[0.02]">
          ...
          <CardContent className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-xs text-muted/70 mb-1">User ID</p>
              <p className="font-mono text-xs text-foreground/75 break-all">{localUser.id}</p>
            </div>
            <div>
              <p className="text-xs text-muted/70 mb-1">Type</p>
              <p className="text-foreground/75 capitalize">{localUser.type}</p>
            </div>
            <div>
              <p className="text-xs text-muted/70 mb-1">Registered</p>
              <p className="text-foreground/75">{localUser.createdAt.toLocaleDateString()}</p>
            </div>
            <div>
              <p className="text-xs text-muted/70 mb-1">$ANSEM Preference</p>
        ...
        <Card className="bg-white/[0.02]">
          ...
          <CardContent className="space-y-3">
            ...
            {communityFeed.map((post) => (
              <div key={post.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-3 space-y-2">
                ...
                {post.tweetUrl && (
                  <a href={post.tweetUrl} target="_blank" rel="noopener noreferrer" className="block rounded-md bg-white/[0.05] p-2 text-xs text-sky-400 hover:text-sky-300">
    ...
    return (
      <div className="space-y-6 max-w-4xl">
        ...
        <Card className="bg-white/[0.02]">
          <CardContent className="p-6">
            <div className="flex items-start gap-5">
              <div className="shrink-0 h-20 w-20 rounded-xl bg-white/10 flex items-center justify-center overflow-hidden border border-white/10">
        ...
        {localAgent.walletAddress && (
          <Card className="bg-white/[0.02]">
            <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm text-muted"><Wallet className="h-4 w-4 text-amber-500" /> Wallet</CardTitle></CardHeader>
            <CardContent>
              <div className="flex items-center gap-3">
                <p className="font-mono text-sm text-foreground/90 break-all">{localAgent.walletAddress}</p>
        ...
        {skills.length > 0 && (
          <Card className="bg-white/[0.02]">
            <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm text-muted"><Shield className="h-4 w-4 text-amber-500" /> Skills ({skills.length})</CardTitle></CardHeader>
