    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Card className="border-zinc-800 bg-zinc-900/50">
        <CardContent className="p-6">
          <div className="flex items-start gap-5">
            <div className="shrink-0 h-20 w-20 rounded-xl bg-zinc-800 flex items-center justify-center overflow-hidden border border-zinc-700">
              {agent.avatarUrl ? <img src={agent.avatarUrl} alt={agent.name} className="h-full w-full object-cover" /> : <Bot className="h-10 w-10 text-amber-500/60" />}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-2xl font-bold text-zinc-50">{agent.name}</h1>
                {agent.status === "running" ? <Badge variant="success">running</Badge> : <Badge variant="secondary">{agent.status}</Badge>}
                {agent.isPublic && <Badge variant="outline" className="text-zinc-400">public</Badge>}
              </div>
              {agent.persona && <p className="mt-2 text-sm text-zinc-400 line-clamp-3">{agent.persona}</p>}
              <div className="mt-3 flex items-center gap-4 text-xs text-zinc-500">
                <span className="flex items-center gap-1"><Cpu className="h-3 w-3" /> {agent.model || "unknown"}</span>
                {createdAt && <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {createdAt.toLocaleDateString()}</span>}
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-zinc-800 bg-zinc-900/50">
          ...
          <CardContent>
            {agent.walletAddress ? (
              <div className="space-y-2">
                <p className="font-mono text-sm text-zinc-200 break-all">{agent.walletAddress}</p>
                <div className="flex gap-2">
                  <a href={`https://solscan.io/account/${agent.walletAddress}`} target="_blank" rel="noopener noreferrer"><Button variant="outline" size="sm" className="text-xs gap-1">Solscan <ExternalLink className="h-3 w-3" /></Button></a>
                  <a href={`https://solana.fm/address/${agent.walletAddress}`} target="_blank" rel="noopener noreferrer"><Button variant="outline" size="sm" className="text-xs gap-1">Solana FM <ExternalLink className="h-3 w-3" /></Button></a>
                </div>
              </div>
        ...
        <Card className="border-zinc-800 bg-zinc-900/50">
          <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm text-zinc-400"><Shield className="h-4 w-4 text-amber-500" /> Skills ({skills.length})</CardTitle></CardHeader>
          <CardContent>
            {skills.length > 0 ? <div className="flex flex-wrap gap-1.5">{skills.map((s: string) => <Badge key={s} variant="ansem" className="text-xs">{s}</Badge>)}</div> : <p className="text-sm text-zinc-500">No skills</p>}
          </CardContent>
        </Card>
      </div>
      <Card className="border-zinc-800 bg-zinc-900/50">
        <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm text-zinc-400"><Bot className="h-4 w-4 text-amber-500" /> Details</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-2 gap-3 text-sm">
          <div><p className="text-xs text-zinc-500 mb-1">Agent ID</p><p className="font-mono text-xs text-zinc-300 break-all">{agent.id}</p></div>
          <div><p className="text-xs text-zinc-500 mb-1">Model</p><p className="text-zinc-300">{agent.model || "—"}</p></div>
        </CardContent>
      </Card>
    </div>
  );
